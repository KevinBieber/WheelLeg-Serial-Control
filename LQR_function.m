...
const_x_matrix = {Iw, Ip, Im, mw, mp, M, g, R, lm};
const_matrix = {0.000783725, 0.0010, 0.37623527867, 0.518, 1.258, 15.378, 9.8, 0.055, 0.14458}; % 对应 Iw Ip Im mw mp M g R lm共九个

x_matrix = {phi, phi_d, x, x_d, theta, theta_d, T, Tp};
solution_matrix = {0, 0, 0, 0, 0, 0, 0, 0}; % 刚开始用

MatQ = [80 0 0 0 0 0; 0 20 0 0 0 0; 0 0 20 0 0 0; 0 0 0 30 0 0; 0 0 0 0 3000 0; 0 0 0 0 0 1];
...
% L_matrix = data_set(:, 1);
% l_matrix = data_set(:, 2);
% lp_matrix = data_set(:, 3);
% alpha_matrix = data_set(:, 4);
% gama_matrix = data_set(:, 5);
phi_0_matrix = data_set(:, 6);

solution = zeros(20, 8);
solution(:,1) = phi_0_matrix;

L_const_x_matrix = {L, l, lp, alpha, gama};
